...
Each source has an independent last-success timestamp. Transient network indicators are not trusted forever when a source stops refreshing: phishing indexes have a 7-day lookup TTL and Feodo C2 IPs have a 36-hour lookup TTL. Malware file SHA-256 values remain valid because they identify immutable file content. The UI reports both the sources reached on the last run and how many sources are currently fresh.

No API key, account token, GitHub secret, or threat-database private signing key is required. Android release APK/AAB signing remains a normal independent Android distribution requirement.
