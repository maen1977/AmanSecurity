...
shared_storage=(ROOT/'app/src/main/java/com/aman/security/protection/SharedStorageScanner.kt').read_text()
quarantine_policy=(ROOT/'app/src/main/java/com/aman/security/security/QuarantinePolicy.kt').read_text()
...
need('shouldAutoQuarantine' in quarantine_policy and 'KNOWN_THREAT' in quarantine_policy,'confirmed_only_quarantine_policy')
need('quarantineManager.quarantine(file, result)' in downloads and 'QuarantineResult.Success' in downloads,'downloads_auto_quarantine')
need('quarantineManager.quarantine(file, result)' in shared_storage and 'QuarantineResult.Success' in shared_storage,'shared_storage_auto_quarantine')
need('activity_download_quarantined' in strings,'quarantine_activity_message')
...
print('REALTIME_ANTIVIRUS_GATE_OK fgs=1 persistent_status=1 notification_permission_guard=1 boot_restore=1 package_monitor=1 downloads_realtime=1 downloads_catchup=1 full_scan=1 confirmed_only_quarantine=1 all_files_antivirus_disclosure=1')
